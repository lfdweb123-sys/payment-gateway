...
export default function MerchantSidebar() {
  ...
  // Mapping des rôles (pour gérer les valeurs en anglais/français)
  ...
  const normalizedRole = getNormalizedRole(teamRole);

  // Vérifier si un item est accessible
  const isAccessible = (item) => {
    // Propriétaire (non membre d'équipe)
    if (!isTeamMember) {
      // Dashboard toujours accessible
      if (item.path === '/dashboard') {
        return true;
      }
      // Paramètres accessible même sans vérification
      if (item.path === '/settings') {
        return true;
      }
      // Autres pages nécessitent vérification
      return merchantData?.verificationStatus === 'approved';
    }
    
    // Membre d'équipe - vérifier selon son rôle
  ...
  // La barre mobile affiche les 4 premiers items
  const bottomItems = menuItems.slice(0, 4);

  // Rendu d'un item de nav (mobile compact)
  const renderNavItem = (item, options = {}) => {
    const { compact = false, onClick } = options;
    ...
    if (!accessible) {
      return (
        <div key={item.path} className={`flex ${compact ? 'flex-col gap-0.5 px-3 py-2 text-xs' : 'items-center gap-3 px-3 py-2.5 text-sm'} font-medium text-gray-300 opacity-40 cursor-not-allowed select-none ${compact ? 'items-center' : ''}`}>
          <Icon size={20} />
    ...
    return (
      <Link
        ...
        to={item.path}
        onClick={onClick}
        className={`flex ${compact ? 'flex-col gap-0.5 px-3 py-2 text-xs' : 'items-center gap-3 px-3 py-2.5 rounded-xl text-sm'} font-medium transition-all ${isActive
            ? compact
              ? 'text-orange-500'
  ...
  // Item vérification pour propriétaire non vérifié
  const verificationItem = (isCollapsed = false, onClose = null) => {
    if (isTeamMember || merchantData?.verificationStatus === 'approved') return null;
    return (
      <Link
        to="/verification"
        onClick={onClose}
        className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all ${location.pathname === '/verification' ? 'bg-amber-50 text-amber-900' : 'text-amber-600 hover:bg-amber-50'
          } ${isCollapsed ? 'justify-center' : ''}`}
      >
        <Shield size={20} />
        {!isCollapsed && <span>Vérification KYC</span>}
        {!isCollapsed && isPending && <span className="ml-auto w-2 h-2 bg-amber-500 rounded-full animate-pulse" />}
      </Link>
    );
  };
    ...
    <>
      ...
      <aside className={`hidden lg:flex flex-col fixed left-0 top-16 bottom-0 bg-white border-r border-gray-200 z-30 transition-all duration-300 ${collapsed ? 'w-20' : 'w-64'}`}>
          ...
          {menuItems.map((item) => {
            ...
            return (
              <Link
                key={item.path}
                to={item.path}
                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all ${isActive ? 'bg-orange-500 text-white' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-100'
        ...
        <div className="p-3 border-t border-gray-100">
          {isTeamMember && normalizedRole && (
            <div className="mb-3 px-3 py-2 text-xs text-gray-500 border-b border-gray-100">
              Rôle : <span className="font-semibold text-gray-700">{normalizedRole}</span>
            </div>
          )}

          <a href="/help" target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-gray-600 hover:text-gray-900 hover:bg-gray-100 transition-all">
            <HelpCircle size={20} />
            {!collapsed && <span>Aide</span>}
          </a>
          <a href="/api-documentation" target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-gray-600 hover:text-gray-900 hover:bg-gray-100 transition-all">
            <FileText size={20} />
      ...
      <div className={`lg:hidden fixed top-0 left-0 bottom-0 w-72 bg-white z-50 shadow-2xl flex flex-col transition-transform duration-300 ease-in-out ${drawerOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="p-3 border-t border-gray-100 space-y-1">
          ...
          <button onClick={() => { setDrawerOpen(false); logout(); }} className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-gray-600 hover:text-red-600 hover:bg-red-50 transition-all w-full">
