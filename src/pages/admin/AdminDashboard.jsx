...
import {
  ...
  TrendingUp, Search, ChevronRight, Activity,
  ArrowUpRight, ChevronLeft, ChevronDown
} from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  if (loading) return (
    <div style={{ display: 'flex', justifyContent: 'center', padding: '80px 0', marginTop: '64px' }}>
  ...
  const txFiltered = stats.recentTransactions.filter(tx => {
    const matchStatus = txStatus === 'Tous' || tx.status === txStatus;
    const matchSearch = !txSearch || 
      tx.provider?.toLowerCase().includes(txSearch.toLowerCase()) ||
  ...
  return (
    <div style={{ 
      ...
      maxWidth: 1200, 
      margin: '0 auto',
      marginTop: '64px' // Compense la hauteur du header fixe
    }}>
      <style>{`
      ...
      <div style={{ 
        ...
        <div>
          <h1 style={{ fontSize: 'clamp(20px, 5vw, 24px)', fontWeight: 900, color: '#0f172a', margin: 0, letterSpacing: '-.02em' }}>
            Administration
          </h1>
      ...
      {/* ── Grille vérifications + actions rapides ── */}
      <div style={{ 
        display: 'grid', 
          ...
          <div style={{ padding: '18px 22px 14px', borderBottom: '1px solid #f8fafc', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ 
                ...
                background: stats.pendingVerifications > 0 ? '#fef3c7' : '#f1f5f9', 
                color: stats.pendingVerifications > 0 ? '#d97706' : '#64748b' 
              }}>
      ...
      <div style={{ background: '#fff', border: '1px solid #f0f0f0', borderRadius: 20, overflow: 'hidden', animation: 'fadeUp .3s ease .15s both' }}>
              ...
              >
                {['Tous', 'completed', 'pending', 'failed'].map(s => (
                  <option key={s} value={s}>
                    {s === 'Tous' ? 'Tous les statuts' : s === 'completed' ? 'Réussi' : s === 'pending' ? 'En cours' : 'Échoué'}
                  </option>
                ))}
              </select>
        ...
        <div className="desktop-table">
          {txPaginated.length > 0 && (
            <div style={{ 
              display: 'grid', 
              gridTemplateColumns: '1.5fr 1fr 1fr 100px 100px 90px', 
              padding: '8px 22px', gap: 12, 
              fontSize: 10, fontWeight: 700, color: '#94a3b8', 
              textTransform: 'uppercase', letterSpacing: '.07em', 
              borderBottom: '1px solid #f8fafc' 
            }}>
              <span>Marchand</span><span>Provider</span><span>Pays / Méthode</span>
          ...
          {txPaginated.length === 0 ? (
            ...
            txPaginated.map((tx, i) => (
              <div key={tx.id} className="adm-row" style={{ 
                display: 'grid', gridTemplateColumns: '1.5fr 1fr 1fr 100px 100px 90px', 
                padding: '12px 22px', gap: 12, 
                borderBottom: i < txPaginated.length - 1 ? '1px solid #f8fafc' : 'none', 
                alignItems: 'center' 
              }}>
            ...
            <div style={{ display: 'flex', gap: 4 }}>
              <button 
                ...
                style={{ 
                  ...
                  opacity: txPage === 1 ? .3 : 1, display: 'flex', alignItems: 'center', justifyContent: 'center' 
                }}>
              ...
              {Array.from({ length: txTotalPages }, (_, i) => i + 1)
                ...
                .map((p, i) => p === '…'
                  ...
                  : <button key={p} onClick={() => setTxPage(p)} style={{ 
                      width: 28, height: 28, borderRadius: 7, border: `1px solid ${txPage===p?'#f97316':'#e2e8f0'}`, 
                      background: txPage===p?'#f97316':'#fff', cursor: 'pointer', fontSize: 12, fontWeight: 700, 
                      color: txPage===p?'#fff':'#374151', display: 'flex', alignItems: 'center', justifyContent: 'center' 
                    }}>{p}</button>
                )
              }
              <button 
                ...
                style={{ 
                  width: 28, height: 28, borderRadius: 7, border: '1px solid #e2e8f0', 
                  background: '#fff', cursor: txPage === txTotalPages ? 'not-allowed' : 'pointer', 
                  opacity: txPage === txTotalPages ? .3 : 1, display: 'flex', alignItems: 'center', justifyContent: 'center' 
                }}>
