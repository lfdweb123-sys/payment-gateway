...
function StatCard({ icon: Icon, iconBg, iconColor, label, value, sub, sparkData, sparkColor, trend }) {
  return (
    ...
    >
      <div style={{ position: 'absolute', right: -16, top: -16, width: 80, height: 80, borderRadius: '50%', background: iconBg, opacity: 0.5 }}/>
      ...
      <div style={{ fontSize: 11, fontWeight: 600, color: '#9ca3af', textTransform: 'uppercase', letterSpacing: '.08em', marginBottom: 4 }}>{label}</div>
      <div style={{ fontSize: 26, fontWeight: 800, color: '#0f172a', letterSpacing: '-.02em', lineHeight: 1.1 }}>{value}</div>
      {sub && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginTop: 6 }}>
          {trend !== undefined && (
            <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 6px', borderRadius: 6, background: trend >= 0 ? '#f0fdf4' : '#fef2f2', color: trend >= 0 ? '#16a34a' : '#dc2626' }}>
...
function MethodIcon({ method }) {
  ...
  return (
    <div style={{ width: 38, height: 38, borderRadius: 10, flexShrink: 0, background: '#f8fafc', border: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 17 }}>
      {emoji}
    </div>
  );
}
...
function TxRowDesktop({ tx, isLast }) {
  return (
    <div className="gd-row" style={{
      display: 'grid', gridTemplateColumns: '1fr 130px 120px 110px 100px',
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0 }}>
        ...
        <div style={{ minWidth: 0 }}>
          ...
          <div style={{ fontSize: 11, color: '#94a3b8', display: 'flex', alignItems: 'center', gap: 4 }}>
            <Globe size={9}/>
            {tx.country?.toUpperCase()} · {tx.createdAt ? format(new Date(tx.createdAt), 'dd MMM HH:mm', { locale: fr }) : '—'}
          </div>
        </div>
      </div>
      {/* Méthode */}
    ...
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: '28px 16px 80px', fontFamily: "'DM Sans', sans-serif" }}>
      <style>{`
        ...
        @media (max-width: 640px) {
          ...
          .gd-tx-mobile    { display: block !important; }
          .gd-filters      { flex-direction: column !important; }
          .gd-filters > *  { width: 100% !important; min-width: unset !important; }
          .gd-filters select { width: 100% !important; box-sizing: border-box; }
        }
      `}</style>
      ...
      {!isVerified && (
        <div style={{
          ...
          <div style={{ width: 40, height: 40, borderRadius: 12, flexShrink: 0, background: isPending ? '#fef3c7' : '#fee2e2', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <div style={{ flex: 1, minWidth: 180 }}>
            ...
            <p style={{ fontSize: 12, color: '#64748b', margin: '2px 0 0' }}>
              {isPending ? "Vos documents sont en cours d'examen. Délai : 24 à 48h." : isRejected ? "Vos documents n'ont pas été acceptés. Veuillez soumettre à nouveau." : "Vérifiez votre identité pour activer votre compte."}
            </p>
          </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28, flexWrap: 'wrap', gap: 12, animation: 'fadeUp .3s ease' }}>
        ...
        {isVerified && (
          <Link to="/providers" style={{ display: 'inline-flex', alignItems: 'center', gap: 7, padding: '9px 18px', borderRadius: 12, fontSize: 13, fontWeight: 600, background: '#fff', border: '1px solid #e2e8f0', color: '#374151', textDecoration: 'none', transition: 'all .15s' }}
      ...
      {/* ── Stat Cards ── */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 14, marginBottom: 28, opacity: !isVerified ? .4 : 1, pointerEvents: !isVerified ? 'none' : 'auto', animation: 'fadeUp .4s ease .05s both' }}>
        <StatCard icon={DollarSign} iconBg="#f0fdf4" iconColor="#16a34a" label="Solde disponible" value={`${stats.balance.toLocaleString('fr-FR')} XOF`} sub="Prêt à retirer" sparkData={last7} sparkColor="#22c55e"/>
        <StatCard icon={CreditCard} iconBg="#eff6ff" iconColor="#2563eb" label="Total transactions" value={stats.totalTransactions.toLocaleString('fr-FR')} sub={`${stats.successRate}% de succès`} sparkData={last7Count} sparkColor="#3b82f6"/>
        <StatCard icon={TrendingUp} iconBg="#fff7ed" iconColor="#f97316" label="Aujourd'hui" value={stats.todayTransactions} sub="transactions ce jour"/>
        <StatCard icon={Activity} iconBg="#fdf4ff" iconColor="#a855f7" label="Taux de succès" value={`${stats.successRate}%`} sub="sur toutes les transactions" sparkData={last7Count.map((v, i) => { const total = stats.recentTransactions.filter(tx => { const d = new Date(); d.setDate(d.getDate() - (6 - i)); return new Date(tx.createdAt).toDateString() === d.toDateString(); }).length; return total ? Math.round((v / total) * 100) : 0; })} sparkColor="#a855f7"/>
      </div>
      ...
      <div style={{ background: '#fff', borderRadius: 20, border: '1px solid #f0f0f0', overflow: 'hidden', opacity: !isVerified ? .4 : 1, pointerEvents: !isVerified ? 'none' : 'auto', animation: 'fadeUp .4s ease .1s both' }}>
        ...
        <div style={{ padding: '20px 20px 16px', borderBottom: '1px solid #f8fafc' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14, flexWrap: 'wrap', gap: 8 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              {hasFilters && (
                <button onClick={resetFilters} style={{ fontSize: 11, color: '#94a3b8', background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}
              ...
              {isVerified && (
                <Link to="/transactions" style={{ fontSize: 12, fontWeight: 600, color: '#f97316', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 4 }}>
          ...
          <div className="gd-filters" style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            <div style={{ position: 'relative', flex: 1, minWidth: 160 }}>
              ...
              <input type="text" value={search} onChange={e => handleFilter(() => setSearch(e.target.value))} placeholder="Rechercher…"
                style={{ width: '100%', paddingLeft: 32, paddingRight: search ? 28 : 12, paddingTop: 8, paddingBottom: 8, border: '1px solid #e2e8f0', borderRadius: 10, fontSize: 12, background: '#f8fafc', outline: 'none', fontFamily: 'inherit', color: '#0f172a', boxSizing: 'border-box', transition: 'border-color .15s' }}
                onFocus={e => e.target.style.borderColor = '#f97316'}
                onBlur={e => e.target.style.borderColor = '#e2e8f0'}
              />
              {search && (
                <button onClick={() => handleFilter(() => setSearch(''))} style={{ position: 'absolute', right: 8, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: '#94a3b8', display: 'flex', padding: 0 }}>
                  <X size={11}/>
                </button>
              )}
            </div>
            ...
            <div style={{ position: 'relative' }}>
              <select value={statusFilter} onChange={e => handleFilter(() => setStatusFilter(e.target.value))} className="gd-filter-select"
                ...
                {STATUS_FILTERS.map(s => <option key={s} value={s}>{s === 'Tous' ? 'Tous les statuts' : STATUS_LABELS[s]}</option>)}
              </select>
            ...
            <div style={{ position: 'relative' }}>
              <select value={providerFilter} onChange={e => handleFilter(() => setProviderFilter(e.target.value))} className="gd-filter-select"
                ...
                {allProviders.map(p => <option key={p} value={p}>{p === 'Tous' ? 'Tous les providers' : p}</option>)}
              </select>
              <ChevronDown size={11} style={{ position: 'absolute', right: 9, top: '50%', transform: 'translateY(-50%)', color: '#94a3b8', pointerEvents: 'none' }}/>
            </div>
          </div>
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1)
                ...
                .map((p, i) =>
                  p === '…'
                    ...
                    : <button key={p} onClick={() => setPage(p)} style={{ width: 30, height: 30, borderRadius: 8, border: `1px solid ${page === p ? '#f97316' : '#e2e8f0'}`, background: page === p ? '#f97316' : '#fff', cursor: 'pointer', fontSize: 12, fontWeight: 700, color: page === p ? '#fff' : '#374151', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all .15s' }}>{p}</button>
                )
              }
              <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page === totalPages}
                style={{ width: 30, height: 30, borderRadius: 8, border: '1px solid #e2e8f0', background: '#fff', cursor: page === totalPages ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: page === totalPages ? .35 : 1 }}>
      ...
      {isVerified && stats.providersCount === 0 && (
        <div style={{ background: 'linear-gradient(135deg, #fff7ed, #fff)', border: '1px solid #fed7aa', borderRadius: 20, padding: '28px', textAlign: 'center', marginTop: 20, animation: 'fadeUp .4s ease .15s both' }}>
          ...
          <Link to="/providers" style={{ background: '#f97316', color: '#fff', padding: '11px 24px', borderRadius: 12, fontSize: 13, fontWeight: 700, textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 7 }}>
      ...
      {!isVerified && (
        <div style={{ background: '#fff', border: '1px solid #f0f0f0', borderRadius: 20, padding: '32px 24px', textAlign: 'center', marginTop: 20, animation: 'fadeUp .4s ease .15s both' }}>
          ...
          <Link to="/verification" style={{ background: '#0f172a', color: '#fff', padding: '11px 24px', borderRadius: 12, fontSize: 13, fontWeight: 700, textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 7 }}>
