...
import {
  Link, Copy, CheckCircle2, Zap, QrCode, Send, Loader,
  Banknote, FileText, Globe, Smartphone, Mail, MessageCircle,
  ChevronDown, X, ExternalLink, Clock
} from 'lucide-react';
...
function QRCodeImage({ url, size = 180 }) {
  ...
  return <img src={qrUrl} alt="QR Code" style={{ width: size, height: size, borderRadius: 12, border: '1px solid #F0F0F0' }} />;
}
...
export default function PaymentLinks() {
  ...
  /* Charger la config du marchand */
  useState(() => {
    (async () => {
      try {
        const snap = await getDoc(doc(db, 'gateway_settings', 'config'));
        if (snap.exists()) setMerchantConfig(snap.data());
      } catch {}
    })();
  }, []);
  ...
  const primaryColor = merchantConfig?.primaryColor || '#FF6B00';
  ...
  const generateLink = async () => {
    ...
    setLoading(true);
    try {
      // Appel à l'API pour générer le token signé
  ...
  return (
    <div style={{
      maxWidth: 750, margin: '0 auto', padding: '24px 20px 48px',
      ...
      <div style={{ display: 'grid', gap: 18 }}>
        ...
        <div style={card}>
          ...
          <div style={{ display: 'grid', gap: 14 }}>
                ...
                >
                  {selectedCountryData ? (
                    <>
                      ...
                      <span style={{ flex: 1, fontSize: 14, color: '#333' }}>{selectedCountryData.name}</span>
                      <span style={{ fontSize: 11, color: '#AAA', background: '#F0F0F0', padding: '2px 7px', borderRadius: 5 }}>
                        {selectedCountryData.currency}
                      </span>
                    </>
                  ) : (
                    <span style={{ color: '#CCC' }}>Tous les pays</span>
                  )}
                    ...
                    <div style={{
                      ...
                      <button
                        ...
                        style={{
                          ...
                          fontSize: 13, color: '#AAA', textAlign: 'left', borderBottom: '1px solid #F5F5F5',
                        }}
                      >
                        🌍 Tous les pays
                      </button>
            ...
            {/* Méthodes de paiement (si pays sélectionné) */}
          ...
          <button
            ...
            style={{
              ...
              opacity: (!amount || !description) ? .5 : 1,
              transition: 'all .2s',
            }}
          >
            {loading ? (
          ...
          <div style={{ ...card, animation: 'fadeUp .3s ease' }}>
            ...
            <div style={{
              ...
              <button
                ...
                style={{
                  ...
                  fontSize: 11, fontWeight: 600, color: copied ? '#00A550' : '#777',
                  transition: 'all .15s',
                }}
              >
            ...
            <button
              ...
              style={{
                ...
                cursor: 'pointer', fontSize: 12, fontWeight: 600, color: '#AAA',
              }}
            >
        ...
        <div style={{
          ...
          <span>
            <strong style={{ color: '#555' }}>Sécurisé :</strong> Le montant est inclus dans un token signé HMAC-SHA256.
            Le client ne peut pas modifier le prix. Le lien expire après 15 minutes.
          </span>
        </div>
      </div>
    </div>
