...
function TopLoadingBar({ visible }) {
  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-50 h-1 bg-transparent overflow-hidden">
      <div className="h-full bg-gray-900 rounded-full" style={{ animation: 'topbar 1.6s ease-in-out infinite' }} />
      <style>{`@keyframes topbar { 0%{width:0;margin-left:0} 50%{width:65%;margin-left:18%} 100%{width:0;margin-left:100%} }`}</style>
    </div>
  );
}
...
export default function GatewayPay() {
  ...
  useEffect(() => {
    ...
    // Charger les paramètres de la passerelle
    getDoc(doc(db, 'gateway_settings', 'config'))
      .then(snap => { if (snap.exists()) setGatewaySettings(prev => ({ ...prev, ...snap.data() })); })
      .catch(() => {});
  ...
  const pollStatus = (id) => {
    ...
    const interval = setInterval(async () => {
      attempts++;
      if (attempts > maxAttempts) { clearInterval(interval); clearInterval(msgInterval); setStatus('failed'); return; }
      try {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.success) { setLoading(false); pollStatus(data.transactionId); }
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 max-w-md w-full p-10 text-center">
          ...
          <div className="bg-gray-50 rounded-2xl p-5">
            ...
            <p className="text-3xl font-bold text-gray-900">{parseFloat(amount).toLocaleString()} <span className="text-lg text-gray-400 ml-2">{countryData?.currency || gatewaySettings.defaultCurrency}</span></p>
          </div>
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 max-w-md w-full p-10 text-center">
          ...
          <button onClick={() => { setStatus(null); setStep(3); }} className="w-full bg-gray-900 text-white font-semibold py-3 rounded-xl hover:bg-gray-800 transition-all">Réessayer</button>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-gray-100 w-full max-w-xl overflow-hidden">
        ...
        <div className={`${theme.header} text-white px-8 py-7`}>
          <div className="flex items-center justify-between mb-5">
            ...
            <div className="flex items-center gap-2">
              {gatewaySettings.logo && <img src={gatewaySettings.logo} alt="" className="h-5 object-contain brightness-200" />}
              {gatewaySettings.companyName && <span className="text-xs text-gray-400">{gatewaySettings.companyName}</span>}
              {!gatewaySettings.companyName && merchant?.name && <span className="text-xs text-gray-400">{merchant.name}</span>}
            </div>
          </div>
          ...
          <div className="text-center mb-6">
            <p className="text-4xl font-bold tracking-tight">
              ...
              <span className="text-xl font-normal text-gray-400 ml-2">{countryData?.currency || gatewaySettings.defaultCurrency}</span>
            </p>
              ...
              return (
                <div key={n} className="flex items-center gap-1">
                  <div className={`flex items-center gap-1.5 transition-opacity ${step >= n ? 'opacity-100' : 'opacity-25'}`}>
                    <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${done ? 'bg-emerald-400 text-white' : current ? 'bg-white text-gray-900' : 'bg-white/20 text-white'}`}>
                      {done ? '✓' : n}
                    </div>
                    <span className="text-xs hidden sm:inline">{label}</span>
                  </div>
                  {i < stepLabels.length - 1 && <div className={`w-6 h-px mx-1 ${done ? 'bg-emerald-400' : 'bg-white/20'}`} />}
                </div>
              );
            })}
          </div>
        ...
        <div className="p-8">
                  ...
                  {countries.map(c => (
                    <button key={c.code} onClick={() => handleSelectCountry(c.code)} className="p-4 rounded-2xl border border-gray-200 text-left hover:border-gray-500 hover:shadow-sm transition-all group">
                      ...
                      <div className="flex items-start justify-between mt-3">
                        <div><p className="text-sm font-semibold text-gray-900 leading-tight">{c.name}</p><p className="text-xs text-gray-400 mt-0.5">{c.currency}</p></div>
          ...
          {step === 2 && countryData && (
            <div>
              <button onClick={() => { setStep(1); setCountryData(null); setCountry(null); }} className="flex items-center gap-1.5 text-sm text-gray-400 hover:text-gray-900 mb-5 transition-colors"><ArrowLeft size={14} /> Changer de pays</button>
              <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-2xl mb-5">
                ...
                <div><p className="text-sm font-semibold text-gray-900">{countryData.name}</p><p className="text-xs text-gray-400">Devise : {countryData.currency}</p></div>
              </div>
              ...
              {countryData.methods?.length > 0 ? (
                ...
                <div className="text-center py-10 bg-gray-50 rounded-2xl text-sm text-gray-400">Aucune méthode disponible pour ce pays</div>
              )}
            </div>
          )}
          ...
          {step === 3 && selectedMethod && (
            <div>
              <button onClick={() => { setStep(2); setSelectedMethod(null); }} className="flex items-center gap-1.5 text-sm text-gray-400 hover:text-gray-900 mb-5 transition-colors"><ArrowLeft size={14} /> Changer de méthode</button>
              <div className="flex items-center gap-4 p-4 bg-gray-50 rounded-2xl mb-6">
                <div className="w-11 h-11 bg-gray-900 rounded-xl flex items-center justify-center text-white shrink-0">{getMethodIcon(selectedMethod.id)}</div>
                <div><p className="text-sm font-semibold text-gray-900">{selectedMethod.name}</p><p className="text-xs text-gray-400">{countryData?.flag} {countryData?.name}</p></div>
              </div>
              <form onSubmit={handleSubmit} className="space-y-5">
                <div><label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Numéro de téléphone</label><input type="tel" value={phoneNumber} onChange={e => setPhoneNumber(e.target.value)} className="w-full px-4 py-3.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-gray-900 outline-none transition-shadow" placeholder="Ex: 229 97 00 00 00" required /></div>
