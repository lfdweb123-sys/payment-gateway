...
import { Wallet, Clock, CheckCircle, XCircle, Send, History, ChevronLeft, ChevronRight } from 'lucide-react';
    ...
    <div className="max-w-4xl mx-auto p-4 sm:p-6 space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
          ...
          <span className="text-xs text-gray-400">{withdrawals.length} retrait{withdrawals.length !== 1 ? 's' : ''}</span>
        </div>
        ...
        {withdrawals.length === 0 ? (
          ...
          <>
            <div className="divide-y divide-gray-50">
              {paginated.map(w => (
                <div key={w.id} className="px-5 py-3 flex items-center justify-between hover:bg-gray-50 transition-colors">
                  <div>
            ...
            {totalPages > 1 && (
              <div className="px-5 py-3 border-t border-gray-100 flex items-center justify-between gap-3 flex-wrap">
                <span className="text-xs text-gray-400">
                  Page {page} / {totalPages} · {withdrawals.length} retrait{withdrawals.length !== 1 ? 's' : ''}
                </span>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShowForm(false)}>
          <div className="bg-white rounded-2xl w-full max-w-md p-6 space-y-4" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              <div>
                <label className="text-xs text-gray-500 uppercase mb-1 block">Montant (XOF)</label>
                <input type="number" value={form.amount} onChange={e => setForm({...form, amount: e.target.value})}
                  ...
                  placeholder="5000" min="100" max={balance} required/>
              </div>
              <div>
                <label className="text-xs text-gray-500 uppercase mb-1 block">Numéro Mobile Money</label>
                <input type="tel" value={form.phone} onChange={e => setForm({...form, phone: e.target.value})}
                  className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-orange-400"
                  placeholder="22997000000" required/>
              </div>
              <div>
                <label className="text-xs text-gray-500 uppercase mb-1 block">Nom du bénéficiaire</label>
                <input type="text" value={form.name} onChange={e => setForm({...form, name: e.target.value})}
                  className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-orange-400"
                  placeholder={user?.displayName || ''}/>
              </div>
              <button type="submit" className="w-full bg-orange-500 text-white font-semibold py-3 rounded-xl hover:bg-orange-600 transition-colors">
