...
/* ─── Envoi de la commission vers le compte FeexPay du propriétaire ── */
async function sendCommissionToOwner(commission) {
  ...
  const phone   = process.env.OWNER_FEEXPAY_PHONE;   // ex: 22997000000
  const network = process.env.OWNER_FEEXPAY_NETWORK || 'mtn'; // mtn | moov | celtiis

  if (!token || !shopId || !phone) {
    console.warn('Commission owner config manquante — commission non envoyée');
    return null;
  }

  // Minimum FeexPay : 100 XOF
  if (commission < 100) {
    console.info(`Commission ${commission} XOF < 100 XOF minimum — non envoyée`);
    return null;
  }

  try {
    const url = `https://api.feexpay.me/api/transactions/public/payout/${network}`;
    const res = await fetch(url, {
      ...
      body: JSON.stringify({
        ...
        phoneNumber: phone,
        description: 'Commission passerelle 1%',
      }),
    });
    const data = await res.json();
    if (!res.ok) {
      console.error('Erreur envoi commission:', data);
      return null;
    }
    return data.reference || data.id || 'ok';
  } catch (err) {
    console.error('Erreur envoi commission:', err.message);
    return null;
  }
}
...
const PROVIDER_CALLS = {
  ...
  lygos: async (config, { amount, phone, email, description }) => {
    const res = await fetch('https://api.lygosapp.com/v1/payments', {
      ...
      body: JSON.stringify({ amount: Math.round(amount), currency: 'XOF', phone, email: email || '', description: (description || 'Paiement').substring(0, 100), callback_url: `${process.env.VITE_APP_URL}/api/webhook/lygos` }),
    });
    const data = await res.json();
    if (!res.ok) return { success: false, error: data.message };
  ...
  mbiyopay: async (config, { amount, phone, description }) => {
    const res = await fetch('https://api.mbiyopay.com/v1/payments', {
      ...
      body: JSON.stringify({ amount: Math.round(amount), phone, description: (description || 'Paiement').substring(0, 100), callback: `${process.env.VITE_APP_URL}/api/webhook/mbiyopay` }),
    });
    const data = await res.json();
    if (!res.ok) return { success: false, error: data.message };
  ...
  qosic: async (config, { amount, phone, description }) => {
    const res = await fetch('https://api.qosic.com/v1/payment/init', {
      ...
      body: JSON.stringify({ merchant_id: config.QOSIC_MERCHANT_ID, amount: Math.round(amount), currency: 'XOF', phone, description: (description || 'Paiement').substring(0, 50), callback_url: `${process.env.VITE_APP_URL}/api/webhook/qosic` }),
    });
    const data = await res.json();
  ...
  hub2: async (config, { amount, phone, email, description }) => {
    const res = await fetch('https://api.hub2.io/v1/payments', {
      ...
      body: JSON.stringify({ amount: Math.round(amount), currency: 'XOF', customer: { phone, email: email || '' }, description: (description || 'Paiement').substring(0, 200), callback_url: `${process.env.VITE_APP_URL}/api/webhook/hub2` }),
    });
    const data = await res.json();
    if (!res.ok) return { success: false, error: data.message };
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const commission = Math.round(amountNum * 0.01); // 1%
    ...
    const result = await callFn(providerConfig, {
      amount: netAmount, phone, email, country, method, description, currency: 'XOF',
    });

    const isCompleted = result.success && result.status === 'SUCCESSFUL';
    ...
    // Si paiement immédiatement complété (cas synchrone)
    if (isCompleted) {
      ...
      // Envoyer la commission 1% sur le compte FeexPay du propriétaire
      const commissionRef = await sendCommissionToOwner(commission);

      // Logger la commission dans Firestore
