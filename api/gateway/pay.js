...
const PROVIDER_CALLS = {
  ...
  cinetpay: async (config, { amount, phone, email, description, method }) => {
    const { ok, data } = await safeFetch('https://api-checkout.cinetpay.com/v2/payment', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ apikey: config.CINETPAY_API_KEY, site_id: config.CINETPAY_SITE_ID, transaction_id: `GW-${Date.now()}`, amount: Math.round(amount), currency: 'XOF', description: (description || 'Paiement').replace(/[#/$_&]/g, '').substring(0, 200), channels: method === 'card' ? 'CREDIT_CARD' : 'MOBILE_MONEY', notify_url: `${process.env.VITE_APP_URL}/api/webhook/cinetpay`, return_url: `${process.env.VITE_APP_URL}/success`, customer_name: 'Client', customer_email: email || 'client@email.com', customer_phone_number: phone || '', lang: 'FR' }) });
