...
/* ── Helper : parse JSON sans planter si réponse HTML ── */
async function safeJson(res) {
  const text = await res.text();
  try { return JSON.parse(text); } catch { return { _raw: text }; }
}

/* ── Envoi commission 1% vers compte FeexPay du propriétaire ── */
async function sendCommissionToOwner(commission) {
  ...
  if (!token || !shopId || !phone) {
    console.warn('Commission owner config manquante — non envoyée');
    return null;
  }
  if (commission < 100) {
    console.info(`Commission ${commission} XOF < 100 minimum FeexPay — non envoyée`);
    return null;
  }
  try {
    const res = await fetch(`https://api.feexpay.me/api/transactions/public/payout/${network}`, {
      method: 'POST',
    ...
    if (!res.ok) { console.error('Erreur commission:', data); return null; }
...
const PROVIDER_CALLS = {
  ...
  feexpay: async (config, { amount, phone, method }) => {
    ...
    const data = await safeJson(res);
    if (!res.ok) return { success: false, error: data.message || data._raw || 'Erreur FeexPay' };
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (result.success && result.status === 'SUCCESSFUL') {
      ...
      // Envoyer la commission 1% sur votre compte FeexPay
      const commissionRef = await sendCommissionToOwner(commission);
      await db.collection('gateway_commissions').add({
        transactionId: txRef.id, merchantId: merchant.id,
        amount: amountNum, commission, commissionRef,
        status: commissionRef ? 'sent' : 'skipped',
